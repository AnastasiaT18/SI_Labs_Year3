...
static const float WEIGHTS[MC_BUF_SIZE] = {40, 30, 15, 10, 5};

MotorConditioner::MotorConditioner(int satMin, int satMax, int rampStep)
...
int MotorConditioner::saturate(int value) {
...
int MotorConditioner::medianFilter(int value) {
    _buf[_bufIndex] = value;
    _bufIndex = (_bufIndex + 1) % MC_BUF_SIZE;
    if (_bufCount < MC_BUF_SIZE) _bufCount++;
...
float MotorConditioner::weightedAverage(int value) {
    if (_bufCount == 0) { lastWeighted = 0.0f; return 0.0f; }

    float weightedSum = 0.0f;
    float totalWeight = 0.0f;
    int idx = (_bufIndex - 1 + MC_BUF_SIZE) % MC_BUF_SIZE;

    for (int i = 0; i < _bufCount; i++) {
        weightedSum += _buf[idx] * WEIGHTS[i];
        totalWeight += WEIGHTS[i];
        idx = (idx - 1 + MC_BUF_SIZE) % MC_BUF_SIZE;
    }
...
int MotorConditioner::ramp(int current, int target) {
    if (current < target) {
        current += _rampStep;
